...
#players #of players
play_one_round <- function(deck_n, players, deck_c = 0) {
...
play_multiple_rounds <- function(deck_n, players, rounds){
  ...
  for (r in 1:rounds) {
    if (r == 1) {
      temp <- play_one_round(deck_n, players)
      setup <- cbind(round = 1, temp$setup)
      lost <- cbind(round = 1, temp$lost)
    } else {
      temp <- play_one_round(deck_n, players, temp$total_cards)
      setup <- rbind(setup, cbind(round = r, temp$setup))
      lost <- rbind(lost, cbind(round = r, temp$lost))
    }
  }
  
  output <- list('last_deck' = temp$total_cards, 'setup' = setup, 'lost' = lost)
